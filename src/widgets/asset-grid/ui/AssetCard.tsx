...
export function AssetCard({ item, onOpenDetail }: AssetCardProps) {
  ...
  const { data: quotes = [] } = useQuotes(
    [item.ticker, item.futuresTicker].filter(Boolean) as string[],
  );
  ...
  const spotQuote = quotes.find((q) => q.symbol === item.ticker);
  const futuresQuote = item.futuresTicker
    ? quotes.find((q) => q.symbol === item.futuresTicker)
    : null;

  const lastChartValue =
    chartData.length > 0 ? chartData[chartData.length - 1].value : 0;
  const price = spotQuote?.regularMarketPrice ?? lastChartValue;
  ...
  return (
    <div className="rounded-xl border border-[#1e1e2e] bg-[#111118] p-3 flex flex-col gap-2 md:cursor-pointer" onDoubleClick={(e) => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <span
            ...
            style={{
              color:
                ASSET_TYPE_COLORS[type as keyof typeof ASSET_TYPE_COLORS] ??
                "#71717a",
            }}
          >
            {type}
        ...
        {futuresQuote && (
          <div className="flex tems-center justify-between">
            ...
            <span className="text-sm tabular-nums text-zinc-400">
              {formatPrice(
                futuresQuote.regularMarketPrice,
                futuresQuote.currency,
              )}
            </span>
          </div>
        )}
      ...
      <div className="flex items-center mt-auto justify-between py-1 pt-3 border-t border-[#1e1e2e]">
        <ExternalLinks ticker={item.ticker} />
        <button
          type="button"
          onClick={onOpenDetail}
