...
import type { TargetRow } from "@/entities/watchlist/model/types";
...
function toDraft(row: TargetRow): DraftRow {
	return { ticker: row.ticker, priceInput: String(row.price) };
}
...
export function TargetsDialog({
	...
	const eligibleTickers = useMemo(() => eligible.map((i) => i.ticker), [eligible]);
	...
	const availableForNew = eligible.filter((i) => !usedTickers.has(i.ticker));
	const canAdd = rows.length < MAX_TARGET_ROWS && availableForNew.length > 0;

	function addRow() {
		if (!canAdd) return;
		setRows((prev) => [...prev, { ticker: availableForNew[0].ticker, priceInput: "" }]);
	}
	...
	function handleSave() {
		...
		for (const r of rows) {
			...
			parsed.push({ ticker: r.ticker, price });
		}
		onSave(parsed);
		onClose();
	}
		...
		<Dialog open={open} onClose={onClose}>
			<div className="p-4 space-y-4">
				...
				<p className="text-[11px] text-zinc-500 leading-tight">
					Track target prices for up to {MAX_TARGET_ROWS} of your watchlist tickers. The card will
					glow when any one is within 2% of its target.
				</p>

				{eligible.length === 0 && (
					<div className="rounded-lg bg-amber-500/10 px-3 py-2 text-[11px] text-amber-300">
						Add at least one price-able ticker (stock, ETF, crypto, etc.) to your watchlist before
						creating targets.
					</div>
				)}
				...
				<div className="space-y-2">
					{rows.map((row, index) => {
						const otherTickers = new Set(rows.filter((_, i) => i !== index).map((r) => r.ticker));
						const options = eligible.filter(
							(i) => i.ticker === row.ticker || !otherTickers.has(i.ticker),
						);
...
function TargetPriceInput({ value, referencePrice, onChange }: TargetPriceInputProps) {
	function stepBy(direction: 1 | -1) {
		const parsed = Number(value);
		const base = Number.isFinite(parsed) && parsed > 0 ? parsed : (referencePrice ?? 1);
