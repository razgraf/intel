...
import type { TargetDirection, TargetRow } from "@/entities/watchlist/model/types";
import {
	MAX_TARGET_ROWS,
	adaptiveStep,
	getTargetDirection,
...
import { ChevronDown, ChevronUp, Plus, TrendingDown, TrendingUp, X } from "lucide-react";
...
export function TargetsDialog({
	...
	function addRow() {
		if (!canAdd) return;
		setRows((prev) => [...prev, { ticker: eligible[0].ticker, priceInput: "", direction: "long" }]);
	}
		...
		<Dialog open={open} onClose={onClose}>
			<div className="p-4 space-y-4">
				...
				<p className="text-[11px] text-zinc-500 leading-tight">
					Track up to {MAX_TARGET_ROWS} price targets across your watchlist. Pick the same ticker
					more than once for multiple levels. The card glows when any target is within 2%.
				</p>
	...
	return (
		<div className="flex items-center gap-px rounded-md bg-zinc-900/60 p-0.5 shrink-0">
			...
			<button
				...
				className={`rounded-sm p-0.5 transition-colors ${
					value === "short" ? "bg-rose-500/20 text-rose-300" : "text-zinc-500 hover:text-zinc-300"
				}`}
			>
